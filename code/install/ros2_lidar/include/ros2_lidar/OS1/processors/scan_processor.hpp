...
public:
  ...
  {
    std::string topic_name = mdata.lidar_ip;
    _pub_pcl = _node->create_publisher<sensor_msgs::msg::PointCloud2>(topic_name, qos);
    ...
    */
    ...
    if(mdata.lidar_vendor == std::string("OLE_2D_V2")){
      ...
      {
        if (_pub_pcl->get_subscription_count() > 0 && _pub_pcl->is_activated()) {
          auto msg_ptr =
          ...
          projector.projectLaser(*msg_ptr, cloud);
          // _pub->publish(std::move(msg_ptr));
          // RCLCPP_INFO(rclcpp::get_logger("ScanProcessor"), "PointCloud2 data size: %zu", cloud.data.size());
          _pub_pcl->publish(cloud);
        }
      });  
    }
    else{
  ...
  {
    _pub_pcl->on_deactivate();
  }

private:
  rclcpp_lifecycle::LifecyclePublisher<sensor_msgs::msg::PointCloud2>::SharedPtr _pub_pcl;
