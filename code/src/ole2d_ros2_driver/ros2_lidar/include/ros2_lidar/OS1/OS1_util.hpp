  ...
  const typename iterator_type::value_type & empty, C && c, F && f)
{
  ...
  return [ = ](const uint8_t * packet_buf, iterator_type it,
     ...
     uint32_t timestamp_integer(0);
     uint32_t timestamp_decimal(0);	//雷达时间戳
     memcpy(&timestamp_integer, packet_buf + 16, sizeof(uint32_t));
     memcpy(&timestamp_decimal, packet_buf + 20, sizeof(uint32_t));
     uint32_t ts = timestamp_integer + timestamp_decimal/4294967296.0;
     int direction=0;	//电机方向
	   uint16_t rpm=0;	//转速
	   memcpy(&rpm, packet_buf + 24, sizeof(uint16_t));
	   direction = static_cast<int>(rpm >> 15);
	   rpm= rpm & 0x7fff;
	   float frequency=0;
     uint16_t num_360;
     memcpy(&num_360, packet_buf + 26, sizeof(uint16_t));
     double angle_increment =(double)360.0/num_360;
     uint16_t start_num;
     memcpy(&start_num, packet_buf + 40, sizeof(uint16_t));
	   double angle_start = start_num * angle_increment;
     uint16_t packet_num;
     memcpy(&packet_num, packet_buf + 42, sizeof(uint16_t));
    
	   frequency = rpm / 60.0;
	   if(azimuth_last > (int)angle_start){
                  if (id_frame > 2){
                    realwidth = id_col;
                    f(ts_last,id_col);   //返回
                  }
            ...
            azimuth_last = (int)angle_start;
            //std::cout << "==== new  data=== "<<std::endl;
           // 3.scan packet
           for (int icol = 0; icol < packet_num; icol++) {
               azimuth = angle_start + angle_increment*icol;
               memcpy(&distance, packet_buf + 48 + 4 * icol, sizeof(uint16_t));
               memcpy(&intensity, packet_buf + 48 + 4 * icol + 2 , sizeof(uint16_t));
	       if(azimuth<0xFFF0){
