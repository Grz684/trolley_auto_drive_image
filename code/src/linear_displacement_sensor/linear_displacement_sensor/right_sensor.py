...
class RightSensor(Node):
    ...
    def setup_modbus_client(self):
        ...
        while True:
            try:
                self.right_client = ModbusClient(
                    method='rtu',
                ...
                if self.right_client.connect():
                    logger.info("连接到右车桥角度传感器成功")
                    self.consecutive_errors = 0  # 重置错误计数
                    break
                else:
                    logger.error("无法连接到右车桥角度传感器，将在 %d 秒后重试", self.connection_retry_interval)
    ...
    def get_dis(self, client):
        try:
            ...
            if response.isError():
                ...
                logger.error("读取右车桥角度传感器错误：%s (连续错误次数: %d)", response, self.consecutive_errors)
...
def main(args=None):
    ...
    while True:
        try:
            ...
            logger.error("右车桥角度传感器节点错误: %s", str(e))
