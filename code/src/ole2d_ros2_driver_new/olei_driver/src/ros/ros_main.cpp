...
int main(int argc, char* argv[])
{
  ...
  num_layers = 0;
  bool watchdog, apply_correction = 0,inverted,ntp;
  ...
  if (!node->has_parameter("inverted"))
  {
    node->declare_parameter("inverted", inverted);
  }
  node->get_parameter("inverted", inverted);

  RCLCPP_INFO(node->get_logger(), "inverted: %d", inverted);

  if (!node->has_parameter("ntp"))
  {
    node->declare_parameter("ntp", ntp);
  }
  node->get_parameter("ntp", ntp);

  RCLCPP_INFO(node->get_logger(), "ntp: %d", ntp);
  ...
  config->inverted = node->get_parameter("inverted").get_parameter_value().get<bool>();
  config->ntp = node->get_parameter("ntp").get_parameter_value().get<bool>();
